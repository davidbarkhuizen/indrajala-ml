...
def test_covers_every_shape_and_single_and_batch_ops():
    ...
    assert {case.shape for case in cases} == {label for label, *_ in demo.DENSE_SHAPES} | {
        label for label, *_ in demo.CONV_SHAPES
    } | {label for label, _side in demo.POOL_SHAPES}
    ...
    pool_ops = {case.op for case in cases if case.shape == demo.POOL_SHAPES[0][0]}
    assert pool_ops == {"forward", "downstream", "forward_batch", "downstream_batch"}
