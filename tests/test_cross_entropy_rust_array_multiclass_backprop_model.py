...
from tests.helpers import (
    ...
    matching_cross_entropy_multiclass_array_backprop_networks,
)
...
def test_randomized_builds_a_usable_network():
    ...
    network = CrossEntropyRustArrayMultiClassBackpropClassifierNetwork.randomized(LAYER_SIZES, DIMENSION, CLASS_COUNT)
...
def test_snapshot_restore_round_trips_weights():
    ...
    assert_array_network_snapshot_restore_round_trip(
        CrossEntropyRustArrayMultiClassBackpropClassifierNetwork, LAYER_SIZES, DIMENSION, CLASS_COUNT
    )
...
def test_save_load_round_trips_weights_and_predictions(tmp_path):
    ...
    assert_array_network_save_load_round_trip(
        network,
        CrossEntropyRustArrayMultiClassBackpropClassifierNetwork.load,
        tmp_path,
        "cross_entropy_rust_array_model.json",
        state,
    )
...
def test_construction_rejects_invalid_arguments():
    ...
    with pytest.raises(AssertionError):
        CrossEntropyRustArrayMultiClassBackpropClassifierNetwork([], DIMENSION, CLASS_COUNT)

    with pytest.raises(AssertionError):
        CrossEntropyRustArrayMultiClassBackpropClassifierNetwork([0], DIMENSION, CLASS_COUNT)

    with pytest.raises(AssertionError):
        CrossEntropyRustArrayMultiClassBackpropClassifierNetwork(LAYER_SIZES, DIMENSION, class_count=1)
