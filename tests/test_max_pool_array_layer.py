...
LAYER_CLS: dict[str, LayerCls] = {"numpy": MaxPoolArrayLayer, "rust": MaxPoolRustArrayLayer}
...
# non-square
PoolShape = tuple[int, int, int, int, int | None]
...
def _tie_heavy_inputs(rng: np.random.Generator, count: int, dimension: int) -> FloatArray:
...
def test_geometry_attributes_follow_from_the_shape(layer_cls: LayerCls, shape: PoolShape):
...
def test_forward_argmax_and_downstream_match_max_pool_layer(backend: Backend, shape: PoolShape, tie_heavy: bool):
    ...
    argmax_batch = to_numpy(array_layer.argmax_batch).reshape(5, -1)
    ...
    for i, x in enumerate(X):
        ...
        np.testing.assert_array_equal(to_numpy(array_layer.argmax).ravel(), argmax_batch[i])
...
def test_an_all_zero_window_picks_slot_0_in_both_implementations(backend: Backend):
    ...
    assert to_numpy(array_layer.argmax).ravel().tolist() == [0]
...
def test_a_partial_tie_picks_the_first_tied_slot_in_row_major_order(backend: Backend):
    ...
    assert to_numpy(array_layer.argmax).ravel().tolist() == [1]
...
def test_overlapping_windows_send_every_won_delta_to_the_shared_input(backend: Backend, layer_cls: LayerCls):
...
def test_hidden_delta_is_the_downstream_gradient_itself_and_gradient_hooks_are_no_ops(
    backend: Backend, layer_cls: LayerCls
):
    ...
    layer.compute_hidden_delta(fixed_downstream(backend, G))
...
def test_constructor_rejects_invalid_arguments(layer_cls: LayerCls, arguments: PoolShape):
...
def test_finite_difference_gradients_through_conv_pool_conv(
    backend: Backend, layer_cls: LayerCls, pool_stride: int | None
):
    ...
    # Any: layers chained within one backend, which a union can't express
    conv_cls: Any = CONV_LAYER_CLS[backend.name]
    conv1 = conv_cls(8, 8, 2, 3, 3)  # -> 3 x 6 x 6
    pool: Any = layer_cls(6, 6, 3, 2, pool_stride)  # -> 3 x 3 x 3 or 3 x 5 x 5
    ...
    def loss() -> float:
        ...
        return float(np.sum(G * to_numpy(A)))

    loss()
    conv2.compute_hidden_delta_batch(fixed_downstream(backend, G))
