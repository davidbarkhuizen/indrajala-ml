...
from tests.helpers import (
    ...
    matching_cross_entropy_array_backprop_networks,
)
...
def _matching_networks(rng: random.Random):
    return matching_cross_entropy_array_backprop_networks(
        rng, CrossEntropyRustArrayBackpropClassifierNetwork, pa.Array, LAYER_SIZES, DIMENSION
    )
...
def test_randomized_builds_a_usable_network():
    ...
    network = CrossEntropyRustArrayBackpropClassifierNetwork.randomized(LAYER_SIZES, DIMENSION)
...
def test_randomized_accepts_and_discards_input_bounds_for_duck_type_compatibility():
    ...
    network = CrossEntropyRustArrayBackpropClassifierNetwork.randomized(LAYER_SIZES, DIMENSION, [(-1.0, 1.0)] * DIMENSION)
    state = tuple(0.1 * i for i in range(DIMENSION))
...
def test_snapshot_restore_round_trips_weights():
    ...
    assert_single_output_array_network_snapshot_restore_round_trip(
        CrossEntropyRustArrayBackpropClassifierNetwork, LAYER_SIZES, DIMENSION
    )
...
def test_save_load_round_trips_weights_and_predictions(tmp_path):
    ...
    assert_single_output_array_network_save_load_round_trip(
        network,
        CrossEntropyRustArrayBackpropClassifierNetwork.load,
        tmp_path,
        "cross_entropy_rust_array_backprop_model.json",
        state,
    )
...
def test_construction_rejects_invalid_arguments():
    ...
    with pytest.raises(AssertionError):
        CrossEntropyRustArrayBackpropClassifierNetwork([], DIMENSION)

    with pytest.raises(AssertionError):
        CrossEntropyRustArrayBackpropClassifierNetwork([0], DIMENSION)
