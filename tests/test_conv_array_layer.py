...
ConvShape = tuple[int, int, int, int, int, int]
...
def _python_forward(conv_layer: ConvLayer, state_layer: StateLayer, x: FloatArray) -> list[float]:
...
def test_geometry_attributes_follow_from_the_shape(layer_cls: LayerCls, shape: ConvShape):
...
@pytest.mark.parametrize("shape", SHAPES)
def test_forward_matches_conv_layer_single_and_batch(backend: Backend, shape: ConvShape):
    ...
    A = to_numpy(array_layer.forward_batch(backend.owned(X.tolist())))
    ...
    for i, x in enumerate(X):
        ...
        a = to_numpy(array_layer.forward(backend.owned(x.tolist())))
        assert a.shape == (array_layer.size,)
        np.testing.assert_array_equal(a, A[i])


@pytest.mark.parametrize("shape", SHAPES)
def test_downstream_matches_conv_layer_downstream_sum_for_every_input(backend: Backend, shape: ConvShape):
    ...
    downstream_batch = to_numpy(array_layer.downstream_batch())
    ...
    for row, deltas in enumerate(delta_batch):
        ...
        downstream = to_numpy(array_layer.downstream())
        assert downstream.shape == (array_layer.input_size,)
        np.testing.assert_array_equal(downstream, downstream_batch[row])


@pytest.mark.parametrize("shape", SHAPES)
def test_hidden_delta_matches_conv_unit_single_and_batch(backend: Backend, shape: ConvShape):
    ...
    array_layer.compute_hidden_delta_batch(fixed_downstream(backend, G))
    delta_batch = to_numpy(array_layer.delta_batch)
    ...
    for i, x in enumerate(X):
        ...
        array_layer.compute_hidden_delta(fixed_downstream(backend, G[i : i + 1]))
        np.testing.assert_array_equal(to_numpy(array_layer.delta), delta_batch[i])


def test_stride_past_the_kernel_leaves_unread_inputs_with_exactly_zero_gradient(backend: Backend):
    ...
    dX = to_numpy(array_layer.downstream_batch()).reshape(2, 2, 6, 6)
...
@pytest.mark.parametrize("shape", SHAPES)
def test_gradient_accumulation_matches_conv_kernel_accumulators_over_a_batch(
    backend: Backend, layer_cls: LayerCls, shape: ConvShape
):
...
def test_relu_derivative_is_zero_at_exactly_z_equals_zero(backend: Backend, layer_cls: LayerCls):
    ...
    if isinstance(layer, ConvArrayLayer):  # numpy
    ...
    layer.compute_hidden_delta_batch(fixed_downstream(backend, np.full((1, 4), 0.5)))
    assert layer.delta_batch.tolist() == [[0.0, 0.5, 0.5, 0.5]]

    layer.compute_hidden_delta(fixed_downstream(backend, np.full((1, 4), 0.5)))
    assert layer.delta.tolist() == [0.0, 0.5, 0.5, 0.5]


def test_output_deltas_are_rejected(backend: Backend, layer_cls: LayerCls):
...
def test_constructor_rejects_invalid_arguments(layer_cls: LayerCls, arguments: ConvShape):
...
@pytest.mark.parametrize("shape", SHAPES)
def test_finite_difference_gradients_on_the_layer_alone(backend: Backend, layer_cls: LayerCls, shape: ConvShape):
    ...
    def loss() -> float:
        ...
        return float(np.sum(G * to_numpy(probe.forward_batch(backend.owned(X.tolist())))))
    ...
    for array, gradient in ((W, grad_W), (b, grad_b), (X, input_gradient)):
        for index in np.ndindex(array.shape):
            ...
            assert gradient[index] == approx((plus - minus) / (2 * eps), abs=1e-6)
