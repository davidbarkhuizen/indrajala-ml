...
Current numbers only. When a change replaces one, the old value moves to
[History](history.md#superseded-numbers) with the PR that replaced it.
...
Not re-measured since candidates 1-4: a demo run after candidate 3 moved cells the change can't
touch by as much as the change itself (see [History](history.md#candidate-3-dense-single-example-downstream-through-the-tiled-kernel)),
so the table keeps these numbers until a run with more repeats replaces them:
...
Dense MNIST 784 -> 30 -> 10, one 60000-example epoch: Rust/numpy about 0.21 single-example and
0.54 mini-batch 32 since candidate 2 (#379). Candidate 2 raised the B = 32 ratio because numpy's
accuracy pass gained more.
...
OpenBLAS's threads may still be spinning (see [Lessons](lessons.md)); that can only slow Rust, by at
...
`downstream` at 32 x 5408 is 0.9x). The gaps left are in batch ops. Rust / numpy µs per
call, focused benchmark (see [How to measure](method.md#how-to-measure)), numpy and Rust in separate processes, Rust
...
batch-512 Rust numbers are partly warm-clock numbers (see [Threading](kernels.md#threading)).
numpy's numbers are with OpenBLAS's default
...
time by op, cProfile over one MNIST epoch (2000 rows), 2026-09-24, before candidates 3 and 4 (the
parentheses give those ops' shares since; the rest is not re-profiled):
...
- **About a quarter of each timed MNIST conv epoch is not training.** The trainers run
  ...
  single-example epoch is similar (4000 of its 6000 conv forwards). numpy ran the same per-row
  passes then (batched since candidate 2), so the ratios include them, and every stake quoted in
  the candidates as a share of an epoch is a share of this timed epoch. That overstates the
  passes for longer runs: the trainers run n + 1 passes for n epochs, so a one-epoch run has two
  and a long run about one per epoch.
- **Conv MNIST, mini-batch 32** (0.74 s): `conv_forward_batch` 27% (its 63 batch calls about 0.95
  ms each since candidate 4, against 0.86 ms for 32 single-example calls),
...
- **Conv MNIST, single-example** (0.80 s): `layer_sgd_step` 20%, `conv_forward_batch` 20%,
  `layer_forward` 19%, dense `downstream` 5-7% since candidate 3, conv accumulate 7%.
