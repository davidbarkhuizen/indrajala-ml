...
def _matching_networks(rng: random.Random, backend):
    return matching_l2_array_backprop_networks(
        rng, NETWORK_CLS[backend.name], backend.owned, LAYER_SIZES, DIMENSION, CLASS_COUNT, L2_LAMBDA
    )
...
def test_randomized_builds_a_usable_network(network_cls):
    ...
    network = network_cls.randomized(LAYER_SIZES, DIMENSION, CLASS_COUNT, L2_LAMBDA)
...
def test_snapshot_restore_round_trips_weights(network_cls):
    ...
    assert_array_network_snapshot_restore_round_trip(network_cls, LAYER_SIZES, DIMENSION, CLASS_COUNT, L2_LAMBDA)
...
def test_save_load_round_trips_the_l2_lambda(network_cls, tmp_path):

    network = network_cls.randomized(LAYER_SIZES, DIMENSION, CLASS_COUNT, l2_lambda=0.02)
...
def test_construction_rejects_invalid_arguments(network_cls):
    ...
    with pytest.raises(AssertionError):
        network_cls([], DIMENSION, CLASS_COUNT, L2_LAMBDA)

    with pytest.raises(AssertionError):
        network_cls([0], DIMENSION, CLASS_COUNT, L2_LAMBDA)

    with pytest.raises(AssertionError):
        network_cls(LAYER_SIZES, DIMENSION, 1, L2_LAMBDA)
