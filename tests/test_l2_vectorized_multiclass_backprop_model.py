...
def test_randomized_builds_a_usable_network():
    ...
    network = L2VectorizedMultiClassBackpropClassifierNetwork.randomized(
...
def test_snapshot_restore_round_trips_weights():
    ...
    # calls assume no further required constructor argument, which doesn't hold here - l2_lambda
    # is required, no default, matching L2RegularizedBackpropClassifierNetwork's own posture.
    network = L2VectorizedMultiClassBackpropClassifierNetwork.randomized(
        LAYER_SIZES, DIMENSION, CLASS_COUNT, L2_LAMBDA
    )
    snapshot = network.snapshot()

    other = L2VectorizedMultiClassBackpropClassifierNetwork(LAYER_SIZES, DIMENSION, CLASS_COUNT, L2_LAMBDA)
...
def test_save_load_round_trips_weights_and_predictions(tmp_path):
    ...
    assert_array_network_save_load_round_trip(
        network,
        L2VectorizedMultiClassBackpropClassifierNetwork.load,
        tmp_path,
        "l2_vectorized_model.json",
        state,
    )
...
def test_save_load_round_trips_the_l2_lambda(tmp_path):
    ...
    path = str(tmp_path / "l2_vectorized_hyperparams.json")
    network.save(path)

    loaded = L2VectorizedMultiClassBackpropClassifierNetwork.load(path)
...
def test_construction_rejects_invalid_arguments():
    ...
    with pytest.raises(AssertionError):
        L2VectorizedMultiClassBackpropClassifierNetwork([], DIMENSION, CLASS_COUNT, L2_LAMBDA)

    with pytest.raises(AssertionError):
        L2VectorizedMultiClassBackpropClassifierNetwork([0], DIMENSION, CLASS_COUNT, L2_LAMBDA)

    with pytest.raises(AssertionError):
        L2VectorizedMultiClassBackpropClassifierNetwork(LAYER_SIZES, DIMENSION, class_count=1, l2_lambda=L2_LAMBDA)
