...
def test_max_pool_ops_match_max_pool_array_layer_exactly(shape, tie_heavy):
    ...
    np.testing.assert_array_equal(_np(A), layer.A)
    np.testing.assert_array_equal(_np(argmax), layer.argmax_batch.reshape(BATCH_SIZE, -1))

    dX = pa.max_pool_downstream_batch(pa.Array(layer.delta_batch.tolist()), argmax, _geometry(layer))
    np.testing.assert_array_equal(_np(dX), layer.downstream_batch())
